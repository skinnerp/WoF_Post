...
### Yellow qualitative colors from Color-Hex (https://www.color-hex.com/color-names.html)

   gold1 = (255/255., 215/255., 0/255.)
   gold2 = (238/255., 201/255., 0/255.)
   gold3 = (205/255., 173/255., 0/255.)
   gold4 = (139/255., 117/255., 0/255.)

   goldenrod1 = (255/255., 193/255., 37/255.)
   goldenrod2 = (238/255., 180/255., 34/255.)
   goldenrod3 = (205/255., 149/255., 12/255.)
   goldenrod4 = (139/255., 105/255., 20/255.)

   yellow1 = (255/255., 255/255., 0/255.)
   yellow2 = (238/255., 238/255., 0/255.)
   yellow3 = (205/255., 205/255., 0/255.)
   yellow4 = (139/255., 139/255., 0/255.)

   beige= (245/255., 245/255., 220/255.)
   
   xparent = (255/255., 255/255., 255/255., 0.01) #last value is alpha   
### Custom Colormaps: 
...
#   nws_dz_cmap = matplotlib.colors.ListedColormap([c5, c10, c15, c20, c25, c30, c35, c40, c45, c50, c55, c60, c65, c70])
   ...
   wind_cmap = matplotlib.colors.ListedColormap([gray1, gray2, gray3, orange2, orange3, orange4, orange5, orange6, orange7, orange8])
...
#   td_cmap = matplotlib.colors.ListedColormap([orange6, orange5, orange4, orange3, gray6, gray5, gray4, gray3, gray2, gray1, green1, green2, green3, green4, green5, green6, blue3, blue4, blue5, blue6, purple3, purple4, purple5 ])
   ...
   mfc_cmap = matplotlib.colors.ListedColormap([blue7, blue6, blue5, blue2, gray1, red2, red5, red6, red7])

   corf_cmap = matplotlib.colors.ListedColormap([purple7, purple5, purple3, purple1, gray1, gray1, orange1, orange3, orange5, orange7])

   ul_dvg_cmap = matplotlib.colors.ListedColormap([green7, green6, green5, green4, green3, gray1, gray1, purple3, purple4, purple5, purple6, purple7])   

   rain_cmap = matplotlib.colors.ListedColormap([green2, green3, green5, blue4, blue5, blue6, purple6, purple5, purple4, red4, red5, red6], 'precipitation')
...
#######################
   ...
   def __init__(self, name, var1_title, var1_levels, var1_cont_levels, var1_cont_colors, over_color, under_color, cmap, extend, alpha, neighborhood):
      self.name                 = name                  #string name for plot (e.g. wz0to2rot90)
      self.var1_title           = var1_title            #string title for var1 plot
      self.var1_levels          = var1_levels           #contour levels for var1 plot
      self.var1_cont_levels     = var1_cont_levels      #levels for second contour plot with var1 (e.g. 60 deg isodrosotherm)
...
#      np.random.shuffle(members)
      ...
      t5 = fig.text(0.07,0.6, 'RAINFALL OBJECTS NOT AVAILABLE!', fontsize=22, fontweight='bold', color=plot.var1_tcolor)
...
########## Plot radmask, var1 and var2 as a pcolormesh, var3 as a shaded contour: #####################
      ...
      p3 = map.contourf(vx, vy, var3, colors=plot.var3_color, levels=plot.var3_level, alpha=0.45) 
...
############ Add text labels of plot titles, initialization and valid forecast time, and OTS info: ###############
      ...
      t5 = fig.text(0.07,0.6, 'RAINFALL OBJECTS NOT AVAILABLE!', fontsize=24, fontweight='bold', color=plot.var1_tcolor)
...
############ Scatter OK mesonet data: #################
   ...
   p5 = map.scatter(ob_x, ob_y, c=ob_var, s=300, linewidth=0.75, edgecolors=cb_colors.gray8, cmap=plot.cmap, alpha=0.55, vmin=np.min(plot.var1_levels), vmax=np.max(plot.var1_levels))
...
def mrms_qpe_plot(map, fig, ax1, ax2, ax3, x, y, plot, var1, t, init_label, valid_label, domain, outdir, t_star, t_plus, showmax='False'):
...
#   temp_outtime = str(t * t_star + t_plus)
   temp_outtime = int(t * t_star + t_plus)
   print("The out_time step is: %s" % (str(temp_outtime)))
   print(len(str(temp_outtime)))
   if (len(str(temp_outtime)) == 1):
      outtime = 'f00%s' % (str(temp_outtime))
   elif (len(str(temp_outtime)) == 3): 
      outtime = 'f%s' % (str(temp_outtime))
   else:
      outtime = 'f0%s' % (str(temp_outtime))
...
def plot_warn(map, fig, ax1, ax2, ax3, shapefile, start_time, end_time, svr_color, tor_color, ff='True', plot_sv='True', plot_to='True', plot_ff='True'):
...
########### If warning is valid at the current forecast time, append plot of polygon to svr/tor variable: ################
   ...
   for info, shape in zip(map.warnings_info, map.warnings):
      if ((info['PHENOM'] == 'SV') and (plot_sv == 'True')):
         ...
         if (init_sec <= 44100.):  ########REMOVE THESE LATER - FUDGE FOR TIMES > 00Z
            init_sec = init_sec + 86400.
            expire_sec = expire_sec + 86400.
         elif (expire_sec <= 44100.):
            expire_sec = expire_sec + 86400.
...
#                  svr.append(map.plot(wx, wy, marker=None, color=svr_color, linewidth=1.75, alpha=0.6))
      ...
      elif ((info['PHENOM'] == 'TO') and (plot_to == 'True')):
         temp_inithr = info['ISSUED'][8:10]
         temp_initmin = info['ISSUED'][10:12]
         init_sec = double(temp_inithr) * 3600. + double(temp_initmin) * 60.
...
#      for info, shape in zip(map.warnings_info, map.warnings):
      ...
      elif ((info['PHENOM'] == 'FF') and (plot_ff == 'True')):
         temp_inithr = info['ISSUED'][8:10]
         temp_initmin = info['ISSUED'][10:12]
         init_sec = double(temp_inithr) * 3600. + double(temp_initmin) * 60.
...
def envqc_plot(map, fig, ax1, ax2, ax3, x, y, plot, var1, var2, obj_var, paint_levels, t, init_label, valid_label, domain, outdir, q_u, q_v, scale, t_star, t_plus, spec, quiv, showmax='False'):
...
############# Create contour plot for var2 data:  #################
   ...
   paint = map.contourf(x, y, obj_var, colors=[cb_colors.gray6, cb_colors.gray6], levels=paint_levels, alpha=0.65, extend='max')
