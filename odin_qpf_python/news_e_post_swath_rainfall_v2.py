...
######################### Find latest available hourly PCP file to speed up realtime processing ###################

for pcp_t in range(1, fcst_nt): # skip first time step
   pcp_found = 0
   str_t = str(pcp_t)
   if (len(str_t) == 1):
      pcp_t = '0' + str_t
   ...
   for f, file in enumerate(summary_files_temp): 
      if ((file[-28:-25] == 'ENS') and (file[-24:-22] == str_t)):                               #assumes filename format of: news-e_ENS_20170516_2200_0000.nc
         pcp_found = 1

   if (pcp_found == 0):
      break
   else:
      if (((pcp_t % 12) == 0) and (pcp_t < fcst_nt)):
         start_t = pcp_t
         time.sleep(2)
...
for f, file in enumerate(summary_files_temp):
   ...
   if (file[-28:-25] == 'PCP'):
      pcp_files.append(file)

ens_files.sort() 
pcp_files.sort()
print(start_t, 'START PCP')

for tt in range(start_t, t+1):

   if (start_t > 0):
      pcp_file = pcp_files[start_t]
      pcp_infile = os.path.join(summary_dir, pcp_file)
   ...
   if (tt == start_t):
      ...
      if (start_t > 0):
          try:
             pcp_fin = netCDF4.Dataset(pcp_infile, "r")
             print("Opening %s \n" % pcp_infile)
          except:
             print("%s does not exist! \n" % pcp_infile)
             sys.exit(1)
      ...
      if (start_t == 0):
         ...
         rain               = pcp_fin.variables["rain_out"][:,:,:]
         comp_dz		 = pcp_fin.variables["comp_dz_out"][:,:,:]
         rain_15m           = pcp_fin.variables["rain_out"][:,:,:]
         rain_hourly        = pcp_fin.variables["rain_out"][:,:,:]
         rain_3hr		 = pcp_fin.variables["rain_out"][:,:,:]
         rain_6hr		 = pcp_fin.variables["rain_out"][:,:,:]
         comp_dz_hourly	 = pcp_fin.variables["comp_dz_out"][:,:,:]
