...
for f, file in enumerate(summary_files_temp):
   if ((file[-28:-25] == 'LTC') and (file[-24:-22] == timestep)):                               #assumes filename format of: news-e_ENS_20170516_2200_0000.nc
      infile = os.path.join(summary_dir, file)

print 'Matched LTC File: ', timestep, '   ', infile
...
ws_levels_kts           = np.arange(22.,154.,6.)                  #(kts)
...
if (name == 'wz_0to2'):
   ...
   base_plot.cmap = cb_colors.wind_trop_cmap  
   base_plot.over_color = cb_colors.purple6 
elif (name == 'comp_dz'):
...
if ((int(valid_hour) < 12) and (int(init_hour) > 15)):
   temp_day = int(day) + 1
...
if (name == 'multi'):
   ...
   rain_15km = fin.variables["rain_two_prob_15km"][edge:-edge,edge:-edge]
   ...
   wind_15km_H1 = fin.variables["ws_80_prob_15km_h1"][edge:-edge,edge:-edge]
...
#   prob_plot.var1_title = prob_plot.var1_title + var_units
   ...
   prob_plot.var1_title = 'Multi-Hazard (Vertical Vorticity [red], Rain [green], Wind [blue]) Prob.'
#   prob_plot.var1_title = prob_plot.var1_title + var_units
   multi_plot(map, fig2, ax21, ax22, ax23, ax24, x, y, prob_plot, var_15km[:,:], wind_15km_H1[:,:], rain_15km[:,:], t, init_label, valid_label, domain, outdir, '', '', '', 5, 0, spec='False', quiv='False')
...
#   prob_plot.var1_title = 'Probability of ' + var_label + ' > %s ' % str(wz_0to2_thresh3)
#   prob_plot.var1_title = prob_plot.var1_title + var_units
...
if (name == 'ws_80'):
   var_50 = fin.variables["ws_80_50"][edge:-edge,edge:-edge]
   ...
   var_15km_ts = fin.variables["ws_80_prob_15km_ts"][edge:-edge,edge:-edge]
   ...
   var_15km_h1 = fin.variables["ws_80_prob_15km_h1"][edge:-edge,edge:-edge]
   ...
   var_15km_h3 = fin.variables["ws_80_prob_15km_h3"][edge:-edge,edge:-edge]
   var_27km_h3 = fin.variables["ws_80_prob_27km_h3"][edge:-edge,edge:-edge]

   base_plot.name = name + '_50'
   base_plot.var1_title = 'Ens. 50th Percentile Value of ' + var_label +  ' (' + var_units + ')'
   env_plot(map, fig, ax1, ax2, ax3, x, y, base_plot, var_50[:,:], pmm_dz[:,:], t, init_label, valid_label, domain, outdir, '', '', '', 5, 0, spec='False', quiv='False', showmax='True')
   ...
   prob_plot.name = name + '_prob_15km_TS'
   prob_plot.var1_title = 'Probability of ' + var_label + ' > %s ' % str(ws_80_thresh_ts)
   prob_plot.var1_title = prob_plot.var1_title + var_units
   env_plot(map, fig, ax1, ax2, ax3, x, y, prob_plot, var_15km_ts[:,:], pmm_dz[:,:], t, init_label, valid_label, domain, outdir, '', '', '', 5, 0, spec='False', quiv='False')
   ...
   prob_plot.name = name + '_prob_15km_H1'
   prob_plot.var1_title = 'Probability of ' + var_label + ' > %s ' % str(ws_80_thresh_h1)
   prob_plot.var1_title = prob_plot.var1_title + var_units
   env_plot(map, fig, ax1, ax2, ax3, x, y, prob_plot, var_15km_h1[:,:], pmm_dz[:,:], t, init_label, valid_label, domain, outdir, '', '', '', 5, 0, spec='False', quiv='False')
   ...
   prob_plot.name = name + '_prob_15km_H3'
   prob_plot.var1_title = 'Probability of ' + var_label + ' > %s ' % str(ws_80_thresh_h3)
   prob_plot.var1_title = prob_plot.var1_title + var_units
   env_plot(map, fig, ax1, ax2, ax3, x, y, prob_plot, var_15km_h3[:,:], pmm_dz[:,:], t, init_label, valid_label, domain, outdir, '', '', '', 5, 0, spec='False', quiv='False')
   ...
   if ((t > 0) and ((t % 12) == 0)):
      var_50 = fin.variables["ws_80_50_hourly"][edge:-edge,edge:-edge]
...
#      var_42km = fin.variables["ws_80_prob_42km_hourly"][edge:-edge,edge:-edge]
      ...
      base_plot.name = name + '_50_hourly'
      base_plot.var1_title = 'Ens. 50th Percentile Value of ' + var_label +  ' (' + var_units + ')'
      env_plot(map, fig, ax1, ax2, ax3, x, y, base_plot, var_50[:,:], pmm_dz[:,:], (t/12), init_label, valid_label, domain, outdir, '', '', '', 1, 0, spec='False', quiv='False', showmax='True')
