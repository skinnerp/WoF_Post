...
while (np.min(process) == 0):
   ...
   for t in fcst_times:
      ...
      for f, file in enumerate(summary_files_temp):
         if ((file[-28:-25] == 'LTC') and (file[-24:-22] == str_t) and (process[t] == 0)):
            process[t] = 1
            for v, variable in enumerate(variables):
               cmd = "python news_e_swath_tc.py -d %s -o %s -m %s -n %s -t %d " % (summary_dir, outdir, mapname, variable, t)
