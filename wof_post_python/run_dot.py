...
asos_dir = os.path.join(base_dir,"asos")
print(flagdir)
...
for itx,hr in enumerate(hours):
    ...
    flagfile = os.path.join(flagdir,'{}_dot.txt'.format(hr))
    tempnt = nt[itx]
...
#    tempfiles = glob.glob(tempsumdir)
    ...
    if len(tempfiles) > 2:
        if os.path.isfile(flagfile) == False:
            #create empty file
            os.mknod(flagfile)
            cmd = '/home/louis.wicker/anaconda2/envs/wof-test/bin/python wrapper_dot.py -d {tempsumdir} -a {asos_dir} -i {imagedir} -e {tempnt}'.format(tempsumdir=tempsumdir,asos_dir=asos_dir,imagedir=imagedir,tempnt=tempnt)
            print 'Executing command: ' + cmd
