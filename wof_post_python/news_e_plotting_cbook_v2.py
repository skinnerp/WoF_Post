...
### Custom Colormaps: 
   ...
   smois_cmap = matplotlib.colors.ListedColormap([orange6, orange5, orange4, orange3, purple3, purple4, purple5, purple6, purple7])
...
#   paintball_colors_list = [q1, b8, q3, q4, q5, q6, q7, q8, b6, q10, q11, b3, b2, purple5, red5, green5, blue5, orange5, q1, b8, q3, q4, q5, q6, q7, q8, b6, q10, q11, b3, b2, purple5, red5, green5, blue5, orange5]
...
########### Save figure as .png, then clear all plots from basemap template: ################
   ...
   fig_name = outdir + plot.name + '_' + outtime + '.png'
   P.savefig(fig_name, format="png", dpi=300, bbox_inches='tight')
#   P.savefig(fig_name, format="png", bbox_inches='tight')
...
########### If plotting ff warnings: ################
      ...
      for info, shape in zip(map.warnings_info, map.warnings):
         if (info['PHENOM'] == 'FF'):
            ...
            if (time <= expire_sec) and (time >= init_sec):
               wx, wy = zip(*shape)
               ff.append(map.plot(wx, wy, marker=None, color=svr_color, linewidth=2., alpha=0.9))
      return ff
...
#            hail.append(map.scatter(wx, wy, s=175, marker='o', facecolor=cb_colors.green6, edgecolor='k', linewidth=0.4, zorder=10))
            hail.append(map.scatter(wx, wy, s=70, marker='o', facecolor=cb_colors.green6, edgecolor='k', linewidth=0.4, zorder=10))
         ...
         if ((init_sec >= init_time) and ((time+150.) >= init_sec)):
            ...
            temp_lon = double(info['LON'])
            wx, wy = map(temp_lon, temp_lat)
#            wind.append(map.scatter(wx, wy, s=175, marker='s', facecolor=cb_colors.blue6, edgecolor='k', linewidth=0.4, zorder=10))
            wind.append(map.scatter(wx, wy, s=70, marker='s', facecolor=cb_colors.blue6, edgecolor='k', linewidth=0.4, zorder=10))
      ...
      if ((info['TYPECODE'] == 'T') and (plot_t == 'True')):
         ...
         if ((init_sec >= init_time) and ((time+150.) >= init_sec)):
            ...
            temp_lon = double(info['LON'])
            wx, wy = map(temp_lon, temp_lat)
#            tornado.append(map.scatter(wx, wy, s=175, marker='v', facecolor=cb_colors.red6, edgecolor='k', linewidth=0.4, zorder=10))
            tornado.append(map.scatter(wx, wy, s=70, marker='v', facecolor=cb_colors.red6, edgecolor='k', linewidth=0.4, zorder=10))
