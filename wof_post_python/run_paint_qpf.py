...
for itx,hr in enumerate(hours):
    ...
    flagfile = os.path.join(flagdir,'{}_paint_qpf.txt'.format(hr))
...
#    tempfiles = glob.glob(tempsumdir)
    ...
    if len(tempfiles) > 2:
        if os.path.isfile(flagfile) == False:
            #create empty file
            os.mknod(flagfile)
            cmd = '/home/louis.wicker/anaconda2/envs/wof-test/bin/python wrapper_paintball_qpf.py -d {tempsumdir} -o {imagedir} -e {tempnt}'.format(tempsumdir=tempsumdir,imagedir=imagedir,tempnt=tempnt)
            print 'Executing command: ' + cmd
